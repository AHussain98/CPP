...
In this phase the preprocessor changes the progrPreprocessingam according to the directives mentioned (that starts with # sign). 
...
The compiler takes the preprocessed file ( without any directives) and generates an object file containing assembly level code. 
Now, the object file created is in the binary form. In the object file created, each line describes one low level machine level instruction.
The created object files can be placed in special archives which we refer to as static libraries, for easier reusing in the future.
But you still can’t run these object files until you convert them into executable file, now here linker comes into play, which links all the object files to generate single executable file. 

Linking

Linking as the name suggests, refers to creation of a single executable file from multiple object files. 
The file created after linking is ready to be loaded into memory and executed by the system. 
There is difference in linking and compilation when it comes to understanding errors. 
